...
This file defines MakeOperators, which is exported in the dll (see dqc_maple.cpp and dqc_python.dll), and supporting functions.
...
note that the rows of mat and mat_ham_sub may be disjoint, which allows the distributed version of this function
...
{
	// if there are secondary machines available, the work of aggregating contributions from all rows will be
	// divided up between multiple machines.  otherwise, DistributedAggregatePotentialContributions just calls
	// AggregatePotentialContributions (in this file).
	tuple<Matrix<double>, Matrix<double>> results = DistributedAggregatePotentialContributions(mat, mat_ham_sub, sigma);
...
* sigma: sigma
* mass: mass
* new_ham: boolean -- whether to use the new or old version of the Hamiltonian
...
Matrix<double> MakeH(Matrix<double>& mat_ham_sub, Matrix<double>& overlaps, Matrix<double>& V0, Matrix<double> sim_t,
	double sigma, double mass, bool new_ham)
{
	...
	{
		v0_coef = 1;
		h0_coef = 1 / (8 * mass * sigma * sigma);
	} // end if/else using new or old version of Hamiltonian
...
Matrix<double> MakeSim(Matrix<double> overlaps)
{
	...
	int start_eig_idx = 0;
	while (eig_vals_full.get(0, start_eig_idx) < 1e-5) { // 2FIX: MOVE 1e-5 CONSTANT TO TOP OF FILE?
		start_eig_idx++;
	}
...
/*
build and return the DQC operators
...
tuple<Matrix<complex<double>>, vector<Matrix<double>>, Matrix<double>> MakeOperators(Matrix<double> mat, int nhambasis, int npotbasis,
																						double sigma, double step, double mass, bool new_ham)
{
	int nrows = mat.rows();
	int ncols = mat.cols();
	...
	Matrix<double> H(MakeH(mat_ham_sub, overlaps, V0, sim_t, sigma, mass, new_ham));
