...
// 2FIX: EVER USED?  DELETE?  REPLACE WITH L2NORMROWS?
void rescaleU(Matrix<double> &U)
{
	...
	for(int y = 0; y < U.rows(); y++)
	{
		...
		for (int x = 0; x < U.cols(); x++)
		{
			U.set(x, y, U.get(x, y) / tempn);
		}
	}
}

/*
	GetXVals gets the expectation values of the list of operators in the states that are the columns of V

	notes
	* the code has been optimized for execution speed.  (most notably, we receive xopvec in flattened 1D
	  form, which speeds things up a lot -- see also AddTrajectories, where the flattening takes place.)
*/
	...
	for (int data_row_idx = 0; data_row_idx < V.cols(); data_row_idx++)
	{
		...
		for (int basis_idx1 = 0; basis_idx1 < num_basis_vecs; basis_idx1++) // loop over all basis rows
		{
			...
			for (int basis_idx2 = 0; basis_idx2 <= basis_idx1; basis_idx2++)
			{
				...
				for (int dim_idx = 0; dim_idx < num_data_dims; dim_idx++) // loop over data dimensions
				{
					// note: we receive xopvec in the appropriately ordererd 1D form (see AddTrajectories), which
...
void RunIter(int current_frame_idx, Array3D &all_frames, Matrix<complex<double>> &combo_op, double *xopvec, 
			int nHambasis, double sigma, int num_data_dims)
{
	int num_batches = int(ceil(double(all_frames.rows()) / double(BATCH_SIZE)));
	Matrix<double> basis_rows(all_frames.view2D(0, all_frames.cols(), 0, nHambasis, 0));
	Array3D current_frame(all_frames.view(0, all_frames.cols(), 0, all_frames.rows(), current_frame_idx, 1));
	
	for (int i = 0; i < num_batches; i++)
	{
		// set up the batch
		int startRow = BATCH_SIZE * i;
		int endRow = i == (num_batches - 1) ? current_frame.rows() : BATCH_SIZE * (i + 1);
		Matrix<double> batch_rows(current_frame.view2D(0, current_frame.cols(), startRow, endRow - startRow, 0));

		// note: 2 normalizations, after conversion and after exponentiation, have both been absorbed into the final renormalization

		// calculate basis overlaps for each row
		Matrix<double> V2evolve(VecsToEvolve(basis_rows, batch_rows, sigma));

		// apply the combined operator (conversion to orthonormal basis states plus Hamiltonian exponentiation)
		// to the overlaps for each row
		Matrix<complex<double>> psinew(combo_op * V2evolve);

		// get new expected position for each row from its new evolved state
		Matrix<double> new_positions(GetXVals(psinew, xopvec, num_data_dims));

		// L2-normalize each row
		rescaleU(new_positions);

		// copy each new position to the final matrix
		for (int y = 0; y < new_positions.rows(); y++)
		{
			for (int x = 0; x < new_positions.cols(); x++)
			{
				all_frames.set(x, startRow + y, current_frame_idx + 1, new_positions.get(x, y));
			}
		}
	} // end for (each batch of rows)
} // end RunIter
...
double* FlattenXops(vector<Matrix<double>> &xops)
{
	...
	IMPORTANT NOTE: xopvec must be deleted by calling code
	*/
...
// 2FIX: CAN DELETE SOON?  (IN FAVOR OF ALWAYS CALLING BUILDFRAMESAUTO -- WITH STOPPING_THRESHOLD SET TO ZERO, IF DESIRED...?)
// 2FIX: NPOTBASIS, STEP, AND MASS ARE NEVER USED HERE
void AddTrajectories(int startFrame, int endFrame, Array3D &all_frames,
					Matrix<complex<double>> &expH, vector<Matrix<double> > &xops, Matrix<double> &simDag,
					int nHambasis, int nPotbasis, double sigma, double step, double mass)
{
	double *xopvec = FlattenXops(xops);

	// create combined operator (conversion to orthonormal basis states plus Hamiltonian eponentiation)
	Matrix<complex<double>> combo_op(expH * simDag);

	for (int i = startFrame; i < endFrame; i++)
	{
		RunIter(i, all_frames, combo_op, xopvec, nHambasis, sigma, xops.size());
	}
	...
	for (int new_frame_idx = 1; new_frame_idx < num_frames; new_frame_idx++)
	{
		...
		if (num_evolving > 0)
		{
			...
			for (int batch_idx = 0; batch_idx < num_batches; batch_idx++)
			{
				...
				// in the 'Understanding DQC' whitepaper, normalization is presented as happening before application of the evolution operator.
				normalizepsi(psi);
				...
				if (rescale)
					rescaleU(new_positions); // apply L2-normalization (project to unit hypersphere)
